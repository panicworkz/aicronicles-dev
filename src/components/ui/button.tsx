...
export const Button = React.forwardRef<HTMLButtonElement, ButtonProps>(
  ({ className, variant = "default", size = "default", ...props }, ref) => {
    const variants = {
      ...
      destructive:
        "bg-destructive/10 text-destructive hover:bg-destructive/20 border border-transparent",
      link: "text-primary underline-offset-4 hover:underline border border-transparent",
    };

    const sizes = {
      default:
        "h-9 gap-2 px-3.5 text-sm font-medium rounded-md [&_svg:not([class*='size-'])]:size-4",
      xs: "h-7 gap-1.5 px-2.5 text-xs font-medium rounded-md [&_svg:not([class*='size-'])]:size-3.5",
      sm: "h-8 gap-1.5 px-3 text-xs font-medium rounded-md [&_svg:not([class*='size-'])]:size-3.5",
      lg: "h-11 gap-2 px-5 text-base font-medium rounded-md [&_svg:not([class*='size-'])]:size-4",
