...
export function BlockInsertToolbar({ title, contentHtml, onInsertHtml }: BlockInsertToolbarProps) {
  ...
  const handleAiAction = async (action: string) => {
    ...
    try {
      ...
      if (data.faqHtml) {
        ...
        toast.success('FAQ Block with Schema.org inserted');
      } else if (data.html) {
        onInsertHtml(data.html);
        toast.success('Block inserted into content');
      }
    } catch (err) {
  ...
  const insertCodeBlock = () => {
    const codeHtml = `
<pre class="my-6 p-4 rounded-xl bg-neutral-900 text-neutral-100 font-mono text-xs overflow-x-auto border border-neutral-800">
...
async function executeWorkflow() {
  ...
  const insertCallout = () => {
    const calloutHtml = `
<div class="my-6 p-4 rounded-xl border border-amber-500/30 bg-amber-500/10 flex items-start gap-3">
  <span class="text-amber-400 font-bold">💡 Pro Tip:</span>
  <p class="text-xs text-neutral-200 m-0">Always verify your primary keyword and ensure your H2 headings directly answer user search intent.</p>
</div>`;
    ...
    <div className="p-3 rounded-xl border bg-muted/30 space-y-2">
      <div className="flex items-center justify-between text-xs font-semibold text-muted-foreground">
        <span className="flex items-center gap-1.5">
          ...
          <span>Rich Interactive Blocks & AI Studio Insert</span>
        </span>
      </div>
